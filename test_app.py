...
class DynamicFormTests(unittest.TestCase):
    def test_new_count_ignores_incomplete_nursing_responses(self):
        records = [("custom", "A1", "فرم آزمایشی", "مدرس")]
        headers = ["شناسه پاسخ دهنده", "نام فارسی", "کد ملی"]
        rows = [
            ["1", "فرد کامل", "0012345678"],
            ["2", "بدون کد ملی", ""],
            ["3", "", "0098765432"],
        ]
        with (
            patch.object(app, "resolve_surveys", return_value={"A1": 1}),
            patch.object(app, "get_form_settings", return_value={"certificate_type": "nursing"}),
            patch.object(app, "fetch_results", return_value=(headers, rows, 3)),
            patch.object(app, "is_processed", return_value=False),
        ):
            self.assertEqual(app.form_new_counts(records), {"A1": 1})

    def test_new_count_requires_gender_for_self_certificates(self):
        records = [("custom", "A1", "فرم آزمایشی", "مدرس")]
        headers = ["شناسه پاسخ دهنده", "نام فارسی", "کد ملی", "جنسیت"]
        rows = [["1", "فرد کامل", "0012345678", "خانم"], ["2", "فرد ناقص", "0098765432", ""]]
        with (
            patch.object(app, "resolve_surveys", return_value={"A1": 1}),
            patch.object(app, "get_form_settings", return_value={"certificate_type": "self"}),
            patch.object(app, "fetch_results", return_value=(headers, rows, 2)),
            patch.object(app, "is_certificate_generated", return_value=False),
        ):
            self.assertEqual(app.form_new_counts(records), {"A1": 1})

    def test_batch_selection_uses_short_id_not_entire_button_label(self):
        batch = {
            "id": 9, "short_id": "R-260815-0001", "row_count": 29,
            "report_names": ["تزریقات اسکالپل"], "nursing_received_at": None,
        }
        state = {"action": "receive", "batch_ids": [9]}
        with (
            patch.object(app, "recent_batches", return_value=[batch]),
            patch.object(app, "set_state"),
            patch.object(app, "send_message") as send,
        ):
            app.process_batch_selection(
                "⏳ R-260815-0001 • 29 نفر • متن دکمه با نمایش متفاوت",
                "7", "8", state,
            )
        self.assertIn("تأیید نهایی", send.call_args.args[0])

    def test_non_owner_cannot_modify_peer_admin_manager(self):
        protected = {"permissions": ["manage_admins"]}
        ordinary = {"permissions": ["view_status"]}
        with patch.object(app, "is_owner", return_value=False):
            self.assertFalse(app.can_modify_admin("2", protected))
            self.assertTrue(app.can_modify_admin("2", ordinary))
        with patch.object(app, "is_owner", return_value=True):
            self.assertTrue(app.can_modify_admin("1", protected))

    def test_postal_contact_fields_are_extracted(self):
        mapping = {
            "نام فارسی": "علی رضایی", "شماره موبایل": "09120000000",
            "کد پستی": "1234567890", "آدرس کامل پستی": "تهران، میدان نمونه",
        }
        person = app.extract_person(mapping)
        self.assertEqual(person["phone_number"], "09120000000")
        self.assertEqual(person["postal_code"], "1234567890")
        self.assertEqual(person["postal_address"], "تهران، میدان نمونه")

    def test_postal_message_only_includes_scalpel_students(self):
        rows = [
            {"teacher_name": "اسکالپل", "persian_name": "علی رضایی", "phone_number": "0912",
             "postal_code": "123", "postal_address": "تهران"},
            {"teacher_name": "مدرس دیگر", "persian_name": "شخص دوم"},
        ]
        with patch.object(app, "send_message") as send:
            app.send_postal_contacts(rows, "8", "آزمایش")
        text = send.call_args.args[0]
        self.assertIn("علی رضایی", text)
        self.assertNotIn("شخص دوم", text)
    ...
    def test_main_menu_is_organized_into_sections(self):
        with patch.object(app, "has_permission", return_value=True):
            labels = [button["text"] for row in app.main_menu("1")["keyboard"] for button in row]
        self.assertIn("⚙️ مدیریت و تنظیمات", labels)
        self.assertIn("🔍 جست‌وجوی افراد", labels)
        self.assertIn("🩺 بررسی سلامت", labels)
        self.assertIn("📚 آموزش کامل کار با ربات", labels)
        self.assertNotIn("👨‍🏫 اسکالپل", labels)
    ...
    def test_custom_form_is_only_in_its_teacher_menu(self):
        ...
        with patch.object(app, "get_form_records", side_effect=forms), \
                patch.object(app, "has_permission", return_value=True):
            first_labels = [button["text"] for row in app.teacher_menu("مدرس الف", "1")["keyboard"] for button in row]
            second_labels = [button["text"] for row in app.teacher_menu("مدرس ب", "1")["keyboard"] for button in row]
