...
def wrapped_line_count(text, width_emu, font_size_pt=24):
    """Estimate PowerPoint wrapping without requiring fonts on the Render server."""
    usable_width = max(1, width_emu - 2 * DEFAULT_TEXT_INSET_EMU)
    # 1 point is 12700 EMU. Microsoft Uighur's average Persian glyph is about
    # 0.32 em wide; weights below keep spaces/digits from being over-counted.
    average_character_width = font_size_pt * 12700 * 0.32
    line_capacity = max(1.0, usable_width / average_character_width)

    def visual_weight(value):
        total = 0.0
    ...
    for word in str(text).split():
        word_width = visual_weight(word)
        space_width = 0.45 if current_width else 0.0
        if current_width and current_width + space_width + word_width > line_capacity:
            lines += 1
...
def position_main_separator(root, main_shape):
    ...
    # Approximate PowerPoint's default 1.2 line spacing at 24pt.
    line_height_emu = round(24 * 12700 * 1.2)
    separator_y = (
