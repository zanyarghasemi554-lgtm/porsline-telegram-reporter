...
def slide_relationships(source_bytes, photo_target, include_notes=True):
    ...
    image_relations = [
        relation for relation in root.findall(f"{{{PR}}}Relationship")
        if relation.get("Type", "").endswith("/image")
    ]
    if len(image_relations) != 1:
        raise RuntimeError("Certificate template must contain exactly one portrait image")
    image_relations[0].set("Target", photo_target)
    return xml_bytes(root)
...
def build(config_path):
    ...
    for index, student in enumerate(students, start=1):
        ...
        photo_name = f"ppt/media/certificate-photo-{index}.png"
        files[slide_name] = replace_slide_tokens(slide_source, student)
        files[rel_name] = slide_relationships(
            rel_source, f"/ppt/media/certificate-photo-{index}.png", include_notes=index == 1
        )
        files[photo_name] = Path(student["portrait_path"]).read_bytes()
        if index == 1:
            continue
