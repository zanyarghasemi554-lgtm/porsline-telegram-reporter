...
def run_report(include_processed=False):
    if not RUN_LOCK.acquire(blocking=False):
        return {"status": "already-running"}
    try:
        require_settings()
        init_db()
        ids = resolve_surveys({INJECTION_SURVEY_CODE, TECHNICIAN_SURVEY_CODE})
        injection, injection_keys, injection_total = collect_new_rows(
            INJECTION_SURVEY_CODE, ids[INJECTION_SURVEY_CODE], include_processed
        )
        technician, technician_keys, technician_total = collect_new_rows(
            TECHNICIAN_SURVEY_CODE, ids[TECHNICIAN_SURVEY_CODE], include_processed
        )
        sent_files = []
        if injection:
    ...
    while True:
        try:
            ...
            for update in telegram_get_updates(offset):
                ...
                valid_commands = {"/report", "/report_all", *SINGLE_REPORT_COMMANDS}
...
def handle_report_command(command):
    ...
    if command == "/report_all":
        send_message("در حال آماده‌سازی گزارش کامل همه پاسخ‌ها…")
    else:
        send_message("در حال آماده‌سازی دو گزارش جدید…")
    result = run_report(include_processed=command == "/report_all")
...
def telegram_webhook():
    ...
    if chat_id == str(TELEGRAM_CHAT_ID) and command in valid_commands:
        threading.Thread(target=handle_report_command, args=(command,), daemon=True).start()
