...
def teacher_menu(teacher_name):
    keyboard = []
    if teacher_name == "اسکالپل":
        keyboard.extend(MY_MENU["keyboard"][:-1])
    elif teacher_name == "خانم طاهرخانی":
        keyboard.extend(TAHER_MENU["keyboard"][:-1])
    elif teacher_name == "خانم ظهیری":
        keyboard.append([{"text": "📄 ثبتی‌های جدید مدارک خانم ظهیری"}])
    for _command, survey_code, report_name in get_custom_forms(teacher_name=teacher_name):
