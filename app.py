...
except ImportError:  # HEIC support is optional during lightweight local tests.
    pass
...
def photo_question(value):
    normalized = clean_header(value)
    return "عکس" in normalized or "تصویر" in normalized or "photo" in normalized or "image" in normalized


def raw_question_value(headers, raw_row):
    labels = [header_label(header, index) for index, header in enumerate(headers)]
    indexes = [index for index, label in enumerate(labels) if photo_question(label)]
    if isinstance(raw_row, list):
        return [raw_row[index] for index in indexes if index < len(raw_row)]
    if not isinstance(raw_row, dict):
        return []
    answers = raw_row.get("values") or raw_row.get("answers") or raw_row.get("cells") or raw_row.get("data") or raw_row.get("row") or raw_row.get("responses")
    found = []
    if isinstance(answers, list):
        found.extend(answers[index] for index in indexes if index < len(answers))
        found.extend(cell for cell in answers if isinstance(cell, dict) and photo_question(header_label(cell, -1)))
    elif isinstance(answers, dict):
        found.extend(value for key, value in answers.items() if photo_question(key))
    found.extend(value for key, value in raw_row.items() if photo_question(key))
    for index in indexes:
        ...
        for key in ("id", "key", "column_id", "object_id", "question_id"):
            ...
            for candidate in (identifier, str(identifier) if identifier is not None else None):
                if candidate is not None and candidate in raw_row:
                    found.append(raw_row[candidate])
    return found


def photo_url_from_value(value):
    preferred_keys = (
        "download_url", "downloadUrl", "file_url", "fileUrl", "url", "src", "link", "path",
    )
    if isinstance(value, dict):
        for key in preferred_keys:
            if value.get(key) not in (None, ""):
                result = photo_url_from_value(value[key])
                if result:
                    return result
        for nested in value.values():
            result = photo_url_from_value(nested)
            if result:
                return result
        return ""
    if isinstance(value, (list, tuple)):
        for item in value:
            result = photo_url_from_value(item)
            if result:
                return result
        return ""
    ...
    match = re.search(r"/(?:media|uploads?|files?|documents?)/[^\s'\"،\\]+", text, re.IGNORECASE)
    if match:
        return urljoin(PORSLINE_BASE_URL + "/", match.group(0))
    return ""
...
def extract_certificate_person(mapping, headers=None, raw_row=None):
    ...
    photo_values = raw_question_value(headers or [], raw_row) if raw_row is not None else []
    photo_values.extend(value for key, value in mapping.items() if photo_question(key))
    photo_url = photo_url_from_value(photo_values)
    person.update({"gender": gender, "photo_url": photo_url})
    return person
...
def download_portrait(url, target_path, depth=0):
    ...
    if "json" in content_type and depth < 2:
        nested_url = photo_url_from_value(response.json())
        if nested_url:
            return download_portrait(nested_url, target_path, depth + 1)
