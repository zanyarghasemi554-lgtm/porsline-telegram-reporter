...
TEMPLATE_PATH = BASE_DIR / "assets" / "report-template.xlsx"
TEMPLATE_B64_PATH = BASE_DIR / "assets" / "report-template.xlsx.b64"
...
PERMISSIONS = {
    "view_status": "مشاهده فرم‌ها و تعداد ثبت‌های جدید",
    "single_reports": "دریافت گزارش تک‌فرمی",
    "multi_reports": "دریافت گزارش چندفرمی",
    "full_reports": "دریافت گزارش کامل همه ثبت‌ها",
    "self_certificates": "ساخت PowerPoint مدارک شخصی",
    "nursing_receive": "ثبت دریافت مدارک از نظام پرستاری",
    "nursing_undo": "اصلاح وضعیت اشتباه مدارک",
    "postal_details": "مشاهده مشخصات پستی اسکالپل",
    "search_people": "جست‌وجوی افراد و وضعیت مدرک",
    "add_forms": "افزودن فرم جدید",
    "edit_forms": "ویرایش و غیرفعال‌کردن فرم",
    "manage_teachers": "افزودن و ویرایش مدرس",
    "teacher_colors": "تغییر رنگ مدرس",
    "transfer_forms": "انتقال فرم بین مدرس‌ها",
    "health": "مشاهده بررسی سلامت ربات",
    "manage_admins": "مدیریت مدیرهای ربات",
}
...
def init_db():
    ...
    with db() as conn, conn.cursor() as cur:
        ...
        cur.execute("ALTER TABLE report_batch_items ADD COLUMN IF NOT EXISTS postal_address TEXT NOT NULL DEFAULT ''")
        cur.execute("ALTER TABLE report_batch_items ADD COLUMN IF NOT EXISTS postal_code TEXT NOT NULL DEFAULT ''")
        cur.execute("ALTER TABLE report_batch_items ADD COLUMN IF NOT EXISTS phone_number TEXT NOT NULL DEFAULT ''")
...
def is_owner(user_id):
    return str(user_id) == str(get_state("bot_owner_user_id", ""))


def get_admin(user_id, active_only=True):
    with db() as conn, conn.cursor() as cur:
        cur.execute(
            "SELECT user_id, display_name, permissions, chat_id, active FROM bot_admins WHERE user_id=%s"
    ...
    if not row:
        return None
    permissions = row[2] if isinstance(row[2], list) else json.loads(row[2] or "[]")
    return {"user_id": row[0], "display_name": row[1], "permissions": permissions,
            "chat_id": row[3], "active": row[4]}
...
def has_permission(user_id, permission):
    ...
    return bool(admin and permission in admin["permissions"])


def require_permission(user_id, permission):
    if not has_permission(user_id, permission):
        raise ValueError(f"⛔ شما اجازه «{PERMISSIONS[permission]}» را ندارید.")


def list_admins(active_only=True):
    with db() as conn, conn.cursor() as cur:
        cur.execute(
            "SELECT user_id, display_name, permissions, chat_id, active FROM bot_admins"
            + (" WHERE active=TRUE" if active_only else "") + " ORDER BY created_at"
        )
        rows = cur.fetchall()
    return [{"user_id": row[0], "display_name": row[1],
             "permissions": row[2] if isinstance(row[2], list) else json.loads(row[2] or "[]"),
             "chat_id": row[3], "active": row[4]} for row in rows]


def save_admin(user_id, display_name, permissions, created_by):
    target_id = str(user_id).strip()
    if not target_id.isdigit():
        raise ValueError("⚠️ آیدی تلگرام باید فقط عدد باشد.")
    if is_owner(target_id):
        raise ValueError("⚠️ این آیدی متعلق به مالک اصلی است.")
    active_admins = list_admins()
    if target_id not in {item["user_id"] for item in active_admins} and len(active_admins) >= 3:
        raise ValueError("⚠️ حداکثر سه مدیر فرعی می‌توانند فعال باشند.")
    valid = sorted(set(permissions) & set(PERMISSIONS))
    with db() as conn, conn.cursor() as cur:
        cur.execute(
            """INSERT INTO bot_admins(user_id, display_name, permissions, created_by, active)
               VALUES(%s,%s,%s::jsonb,%s,TRUE)
               ON CONFLICT(user_id) DO UPDATE SET display_name=EXCLUDED.display_name,
               permissions=EXCLUDED.permissions, active=TRUE""",
            (target_id, str(display_name).strip()[:80], json.dumps(valid), str(created_by)),
        )
        conn.commit()


def can_modify_admin(actor_id, target):
    if is_owner(actor_id):
        return True
    return "manage_admins" not in target["permissions"]


def deactivate_admin(actor_id, target_id):
    target = get_admin(target_id)
    if not target:
        raise ValueError("⚠️ مدیر فعال موردنظر پیدا نشد.")
    if not can_modify_admin(actor_id, target):
        raise ValueError("⛔ مدیر دارای مجوز مدیریت مدیرها فقط توسط مالک اصلی قابل برکناری است.")
    with db() as conn, conn.cursor() as cur:
        cur.execute("UPDATE bot_admins SET active=FALSE WHERE user_id=%s", (str(target_id),))
        conn.commit()


def update_admin_chat(user_id, chat_id):
    if is_owner(user_id):
        set_state("bot_owner_chat_id", chat_id)
        return
    with db() as conn, conn.cursor() as cur:
        cur.execute("UPDATE bot_admins SET chat_id=%s WHERE user_id=%s AND active=TRUE", (str(chat_id), str(user_id)))
        conn.commit()
...
def extract_person(mapping):
    ...
    postal_address = find_value(mapping, [
        "آدرس کامل پستی", "آدرس پستی", "نشانی کامل پستی", "نشانی پستی",
        "آدرس محل سکونت", "نشانی محل سکونت", "آدرس کامل", "نشانی کامل", "آدرس", "نشانی",
    ])
    postal_code = find_value(mapping, ["کد پستی", "کدپستی", "postal code", "postcode"])
    phone_number = find_value(mapping, [
        "شماره تماس", "شماره تلفن همراه", "شماره همراه", "شماره موبایل",
        "تلفن همراه", "موبایل", "تماس", "تلفن", "mobile", "phone number", "phone",
    ])
    return {
...
def extract_certificate_person(mapping, headers=None, raw_row=None):
...
def build_certificate_powerpoint(students, course_title):
    ...
    try:
        ...
        filename = f"مدارک - {safe_filename(course_title)} - {dated} - {len(students)} نفر.pptx"
...
def collect_certificate_rows(code, survey_id, settings):
    ...
    for raw_row in rows:
        ...
        if is_certificate_generated(code, key):
            continue
        person = extract_certificate_person(mapping, headers, raw_row)
...
def run_certificate_export(survey_codes, output_mode="combined", chat_id=None):
    ...
    try:
        ...
        for settings, students in groups:
            if not students:
                continue
            filename, stream = build_certificate_powerpoint(students, settings["course_title"])
            caption = (
                f"🎓 مدارک آماده چاپ\nتعداد: {len(students)} نفر\n"
                f"فرم‌ها: {'، '.join(dict.fromkeys(row['report_name'] for row in students))}\n"
            ...
            record_certificate_batch(
                students, list(dict.fromkeys(row["report_name"] for row in students)), output_mode
            )
            mark_processed([(row["survey_code"], row["response_key"]) for row in students])
            send_postal_contacts(students, chat_id, "مدارک صادرشده توسط شما")
...
def load_report_template():
    """Load the Excel report template without depending on one fragile file format."""
    if TEMPLATE_PATH.exists():
        try:
            template_bytes = TEMPLATE_PATH.read_bytes()
            if not template_bytes.startswith(b"PK"):
                raise ValueError("The XLSX template does not have a valid ZIP signature.")
            return load_workbook(io.BytesIO(template_bytes))
        except Exception:
            log.exception("Could not load binary report template: %s", TEMPLATE_PATH)

    if TEMPLATE_B64_PATH.exists():
        try:
            encoded = TEMPLATE_B64_PATH.read_text(encoding="ascii").strip()
            template_bytes = base64.b64decode(encoded, validate=True)
            if not template_bytes.startswith(b"PK"):
                raise ValueError("The decoded XLSX template does not have a valid ZIP signature.")
            return load_workbook(io.BytesIO(template_bytes))
        except Exception:
            log.exception("Could not load Base64 report template: %s", TEMPLATE_B64_PATH)

    # Last-resort fallback: create a valid workbook instead of crashing the callback.
    log.warning(
        "Report template was unavailable; using a generated blank workbook. Checked: %s and %s",
        TEMPLATE_PATH,
        TEMPLATE_B64_PATH,
    )
    return Workbook()
    ...
    with db() as conn, conn.cursor() as cur:
        ...
        cur.executemany(
            ...
            INSERT INTO report_batch_items(
                ...
                color_key, persian_name, english_name, national_id,
                postal_address, postal_code, phone_number, row_position
            ...
            [(
                ...
                row.get("national_id", ""), row.get("postal_address", ""),
                row.get("postal_code", ""), row.get("phone_number", ""), position,
    ...
    with db() as conn, conn.cursor() as cur:
        cur.execute(
            ...
            SELECT survey_code, response_key, report_name, teacher_name, color_key,
                   persian_name, english_name, national_id, postal_address, postal_code, phone_number
        ...
        return [{
            ...
            "postal_address": row[8], "postal_code": row[9], "phone_number": row[10],
        } for row in cur.fetchall()]


def enrich_batch_contacts(batch_id, items):
    missing_codes = {
        row["survey_code"] for row in items
        if row.get("teacher_name") == "اسکالپل"
        and not (row.get("postal_address") and row.get("postal_code") and row.get("phone_number"))
    }
    if not missing_codes:
        return items
    try:
        ids = resolve_surveys(missing_codes)
        by_identity = {(row["survey_code"], row["response_key"]): row for row in items}
        updates = []
        for code in missing_codes:
            headers, rows, _total = fetch_results(ids[code])
            for raw_row in rows:
                mapping = row_to_mapping(headers, raw_row)
                target = by_identity.get((code, response_key(mapping)))
                if not target:
                    continue
                person = extract_person(mapping)
                for field in ("postal_address", "postal_code", "phone_number"):
                    if person.get(field):
                        target[field] = person[field]
                updates.append((target["postal_address"], target["postal_code"], target["phone_number"],
                                batch_id, code, target["response_key"]))
        if updates:
            with db() as conn, conn.cursor() as cur:
                cur.executemany(
                    """UPDATE report_batch_items SET postal_address=%s, postal_code=%s, phone_number=%s
                       WHERE batch_id=%s AND survey_code=%s AND response_key=%s""",
                    updates,
                )
                conn.commit()
    except Exception:
        log.exception("Could not enrich postal contacts for batch %s", batch_id)
    return items


def send_postal_contacts(rows, chat_id=None, reason="", user_id=None):
    user_id = str(user_id or getattr(REQUEST_CONTEXT, "user_id", ""))
    if user_id and not has_permission(user_id, "postal_details"):
        return
    scalpel_rows = [row for row in rows if str(row.get("teacher_name") or "").strip() == "اسکالپل"]
    if not scalpel_rows:
        return
    blocks = []
    for index, row in enumerate(scalpel_rows, start=1):
        blocks.append(
            f"{index}. 👤 {row.get('persian_name') or 'نام ثبت نشده'}\n"
            f"📱 شماره تماس: {row.get('phone_number') or 'ثبت نشده'}\n"
            f"📮 کد پستی: {row.get('postal_code') or 'ثبت نشده'}\n"
            f"🏠 آدرس کامل: {row.get('postal_address') or 'ثبت نشده'}"
        )
    header = "📦 مشخصات پستی افراد اسکالپل"
    if reason:
        header += f"\nمربوط به: {reason}"
    message = header
    for block in blocks:
...
def current_chat_id():
    return str(getattr(REQUEST_CONTEXT, "chat_id", "") or owner_chat_id())


def send_document(filename, stream, caption, chat_id=None, mime_type=None):
    response = requests.post(
        f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendDocument",
        data={"chat_id": chat_id or current_chat_id(), "caption": caption},
...
def send_message(text, chat_id=None, reply_markup=None):
    payload = {"chat_id": chat_id or current_chat_id(), "text": text}
...
def main_menu(user_id=None):
    user_id = str(user_id or getattr(REQUEST_CONTEXT, "user_id", "") or get_state("bot_owner_user_id", ""))
    keyboard = []
    if any(has_permission(user_id, item) for item in ("view_status", "single_reports", "multi_reports", "full_reports")):
        keyboard.append([{"text": "📊 گزارش‌ها"}])
    if any(has_permission(user_id, item) for item in ("self_certificates", "nursing_receive", "nursing_undo")):
        keyboard.append([{"text": "🎓 مدیریت مدارک"}])
    keyboard.append([{"text": "📋 فرم‌ها و مدرس‌ها"}])
    if has_permission(user_id, "search_people"):
        keyboard.append([{"text": "🔍 جست‌وجوی افراد"}])
    if any(has_permission(user_id, item) for item in (
        "add_forms", "edit_forms", "manage_teachers", "teacher_colors", "transfer_forms", "manage_admins"
    )):
        keyboard.append([{"text": "⚙️ مدیریت و تنظیمات"}])
    keyboard.append([{"text": "📚 آموزش کامل کار با ربات"}])
    if has_permission(user_id, "health"):
        keyboard.append([{"text": "🩺 بررسی سلامت"}])
    keyboard.append([{"text": "✖️ بستن منو"}])
    return {"keyboard": keyboard, "resize_keyboard": True, "is_persistent": True}


def reports_menu(user_id):
    keyboard = []
    if has_permission(user_id, "view_status"):
        keyboard.append([{"text": "📊 وضعیت فرم‌ها"}])
    if has_permission(user_id, "single_reports"):
        keyboard.append([{"text": "📄 گزارش تک‌فرمی"}])
    if has_permission(user_id, "multi_reports"):
        keyboard.append([{"text": "☑️ انتخاب چند فرم"}])
    if has_permission(user_id, "full_reports"):
        keyboard.append([{"text": "📦 گزارش همه فرم‌ها"}])
    keyboard.append([{"text": "🏠 منوی اصلی"}])
    return {"keyboard": keyboard, "resize_keyboard": True, "is_persistent": True}


def forms_teachers_menu():
    teachers = get_teachers()
    keyboard = [[{"text": f"👨‍🏫 {name}"}] for name in teachers]
    keyboard.append([{"text": "🏠 منوی اصلی"}])
    return {"keyboard": keyboard, "resize_keyboard": True, "is_persistent": True}


def certificates_menu(user_id):
    keyboard = []
    if has_permission(user_id, "nursing_receive") or has_permission(user_id, "nursing_undo"):
        keyboard.append([{"text": "🎓 مدارک نظام پرستاری"}])
    if has_permission(user_id, "self_certificates"):
        keyboard.append([{"text": "🖥 مدارک صادره توسط خودم"}])
    keyboard.append([{"text": "🏠 منوی اصلی"}])
    return {"keyboard": keyboard, "resize_keyboard": True, "is_persistent": True}


def teacher_menu(teacher_name, user_id=None):
    user_id = str(user_id or getattr(REQUEST_CONTEXT, "user_id", ""))
    keyboard = []
    ...
    if has_permission(user_id, "manage_teachers"):
        keyboard.extend([
...
def management_menu(user_id):
    keyboard = []
    if has_permission(user_id, "add_forms"):
        keyboard.append([{"text": "➕ ثبت فرم جدید"}])
    if has_permission(user_id, "manage_teachers"):
        keyboard.append([{"text": "➕ افزودن مدرس"}])
    if has_permission(user_id, "teacher_colors"):
        keyboard.append([{"text": "🎨 رنگ مدرس‌ها"}])
    if has_permission(user_id, "transfer_forms"):
        keyboard.append([{"text": "🔄 انتقال فرم بین مدرس‌ها"}])
    if has_permission(user_id, "edit_forms") or has_permission(user_id, "manage_teachers"):
        keyboard.append([{"text": "♻️ بازیابی موارد غیرفعال"}])
    if has_permission(user_id, "manage_admins"):
        keyboard.append([{"text": "👥 مدیریت مدیرها"}])
    keyboard.append([{"text": "🏠 منوی اصلی"}])
    return {"keyboard": keyboard, "resize_keyboard": True, "is_persistent": True}
...
ADMIN_MENU = {
    "keyboard": [
        [{"text": "➕ افزودن مدیر"}],
        [{"text": "✏️ ویرایش دسترسی مدیر"}],
        [{"text": "👤 فهرست و برکناری مدیرها"}],
...
HELP_MENU = {
    "keyboard": [
        [{"text": "📊 آموزش دریافت گزارش‌ها"}],
        [{"text": "🎓 آموزش مدارک نظام پرستاری"}],
        [{"text": "🖥 آموزش ساخت PowerPoint"}],
        [{"text": "📋 آموزش فرم‌ها و مدرس‌ها"}],
        [{"text": "🔍 آموزش جست‌وجوی افراد"}],
        [{"text": "📦 آموزش مشخصات پستی"}],
        [{"text": "👥 آموزش مدیرها و دسترسی‌ها"}],
        [{"text": "⚙️ آموزش بخش مدیریت"}],
        [{"text": "❓ رفع خطاهای متداول"}],
        [{"text": "🏠 منوی اصلی"}],
    ], "resize_keyboard": True, "is_persistent": True,
}
...
HELP_TEXTS = {
    "📊 آموزش دریافت گزارش‌ها": "📊 گزارش‌ها\n\nبرای دیدن تعداد جدیدها «وضعیت فرم‌ها» را بزنید. برای یک فرم از «گزارش تک‌فرمی» و برای چند فرم از «انتخاب چند فرم» استفاده کنید. قبل از ساخت، ربات تأیید نهایی می‌گیرد.",
    "🎓 آموزش مدارک نظام پرستاری": "🎓 مدارک نظام پرستاری\n\nخروجی ثبت‌های جدید به معنی ارسال به نظام است. وقتی مدارک برگشت، «مدارک از نظام رسید» را بزنید و گزارش درست را انتخاب کنید. انتخاب اشتباه قابل اصلاح است.",
    "🖥 آموزش ساخت PowerPoint": "🖥 ساخت PowerPoint\n\nفرم از نوع «خودم صادر می‌کنم» را انتخاب و «ساخت PowerPoint مدارک جدید» را بزنید. هر دانشجو یک اسلاید دارد. بعد از ارسال موفق، وضعیت چاپخانه ثبت می‌شود.",
    "📋 آموزش فرم‌ها و مدرس‌ها": "📋 فرم‌ها و مدرس‌ها\n\nابتدا مدرس و سپس فرم را انتخاب کنید. نوع مدرک، عنوان دوره و اطلاعات روی مدرک از بخش مدیریت قابل ثبت و ویرایش است.",
    "🔍 آموزش جست‌وجوی افراد": "🔍 جست‌وجو\n\nنام فارسی، نام انگلیسی یا کد ملی را وارد کنید تا فرم، مدرس، تاریخ ارسال و وضعیت مدرک نمایش داده شود.",
    "📦 آموزش مشخصات پستی": "📦 مشخصات پستی\n\nبرای فرم‌های اسکالپل، پس از دریافت از نظام یا ساخت مدرک شخصی، نام، تلفن، کد پستی و آدرس به‌صورت پیام مرتب ارسال می‌شود.",
    "👥 آموزش مدیرها و دسترسی‌ها": "👥 مدیرها\n\nحداکثر سه مدیر فرعی با آیدی عددی تلگرام قابل ثبت‌اند. دسترسی هر مدیر با دکمه‌های جدا تعیین می‌شود. مالک اصلی همیشه دسترسی کامل دارد.",
    "⚙️ آموزش بخش مدیریت": "⚙️ مدیریت\n\nدر این بخش می‌توانید فرم و مدرس اضافه کنید، نام‌ها و رنگ را تغییر دهید، فرم را منتقل یا غیرفعال کنید و مدیرها را مدیریت کنید.",
    "❓ رفع خطاهای متداول": "❓ خطاهای متداول\n\nاگر عملیات طول کشید، ابتدا منتظر پایان آن بمانید. اگر فایل ساخته نشد، Logs در Render را بررسی کنید. نبودن قالب PowerPoint یا عنوان‌های ناقص پرس‌لاین از خطاهای رایج است.",
}


def admin_permissions_menu(selected=None):
    selected = set(selected or [])
    keyboard = []
    for key, label in PERMISSIONS.items():
        keyboard.append([{"text": ("✅ " if key in selected else "⬜ ") + label,
                          "callback_data": f"adminperm:toggle:{key}"}])
    keyboard.extend([
        [{"text": "✅ انتخاب همه", "callback_data": "adminperm:all"},
         {"text": "🧹 حذف همه", "callback_data": "adminperm:clear"}],
        [{"text": "💾 ذخیره دسترسی‌ها", "callback_data": "adminperm:save"}],
        [{"text": "❌ انصراف", "callback_data": "cancel"}],
    ])
    return {"inline_keyboard": keyboard}


def admins_remove_menu(actor_id):
    keyboard = []
    for admin in list_admins():
        protected = not is_owner(actor_id) and "manage_admins" in admin["permissions"]
        label = f"{'🔒' if protected else '🗑'} {admin['display_name']} • {admin['user_id']}"
        callback = "admin:protected" if protected else f"admin:remove:{admin['user_id']}"
        keyboard.append([{"text": label, "callback_data": callback}])
    keyboard.append([{"text": "❌ انصراف", "callback_data": "cancel"}])
    return {"inline_keyboard": keyboard}


def admins_edit_menu(actor_id):
    keyboard = []
    for admin in list_admins():
        protected = not can_modify_admin(actor_id, admin)
        label = f"{'🔒' if protected else '✏️'} {admin['display_name']} • {admin['user_id']}"
        callback = "admin:protected_edit" if protected else f"admin:edit:{admin['user_id']}"
        keyboard.append([{"text": label, "callback_data": callback}])
    keyboard.append([{"text": "❌ انصراف", "callback_data": "cancel"}])
    return {"inline_keyboard": keyboard}


def process_admin_setup(text, user_id, chat_id, setup):
    require_permission(user_id, "manage_admins")
    if setup.get("step") == "user_id":
        target_id = normalize_digits(text).strip()
        if not target_id.isdigit():
            send_message("⚠️ آیدی عددی تلگرام را وارد کنید؛ مثلاً 123456789.", chat_id)
            return
        setup.update({"step": "name", "target_id": target_id})
        set_state(f"admin_setup:{user_id}", json.dumps(setup, ensure_ascii=False))
        send_message("📝 یک نام برای این مدیر وارد کنید؛ مثلاً مسئول مدارک:", chat_id)
        return
    if setup.get("step") == "name":
        name = str(text or "").strip()
        if not name or len(name) > 80:
            send_message("⚠️ نام مدیر باید بین ۱ تا ۸۰ کاراکتر باشد.", chat_id)
            return
        setup.update({"step": "permissions", "display_name": name, "selected": []})
        set_state(f"admin_setup:{user_id}", json.dumps(setup, ensure_ascii=False))
        send_message("🔐 دسترسی‌های این مدیر را انتخاب کنید:", chat_id, admin_permissions_menu())
...
def batch_id_from_button_text(text):
    """Extract the stable report id instead of comparing the whole Telegram label."""
    match = re.search(r"\bR-\d{6}-\d+\b", str(text or ""))
...
def custom_form_menu(survey_code, user_id=None):
    user_id = str(user_id or getattr(REQUEST_CONTEXT, "user_id", ""))
    settings = get_form_settings(survey_code)
    keyboard = []
    if settings and settings["certificate_type"] == "self":
        if has_permission(user_id, "self_certificates"):
            keyboard.append([{"text": "🎓 ساخت PowerPoint مدارک جدید"}])
    else:
        if has_permission(user_id, "single_reports"):
            keyboard.append([{"text": "🆕 فقط ثبت‌های جدید"}])
        if has_permission(user_id, "full_reports"):
            keyboard.append([{"text": "📚 همه ثبت‌ها"}])
    if has_permission(user_id, "edit_forms"):
        keyboard.extend([
            [{"text": "✏️ ویرایش نام فرم"}],
            [{"text": "⚙️ ویرایش مشخصات مدرک"}],
            [{"text": "🗑 غیرفعال‌کردن فرم"}],
        ])
    keyboard.append([{"text": "🔙 بازگشت به مدرس"}])
    return {
        "keyboard": keyboard,
        "resize_keyboard": True,
        "is_persistent": True,
...
def form_new_counts(records):
    ...
    for _source, code, _name, _teacher in records:
        ...
        for raw_row in rows:
            ...
            # Count only responses that can actually appear in the requested
            # output. Previously incomplete answers were shown as "new", but
            # collect_new_rows/collect_certificate_rows skipped them later.
            person = extract_certificate_person(mapping, headers, raw_row)
            complete = bool(person.get("persian_name") and person.get("national_id"))
            if settings["certificate_type"] == "self":
                complete = complete and bool(certificate_honorific(person.get("gender")))
            if complete:
                count += 1
        counts[code] = count
    return counts
...
def process_deactivation_callback(data, user_id, chat_id):
    ...
    if pending.get("kind") == "teacher":
        require_permission(user_id, "manage_teachers")
    else:
        require_permission(user_id, "edit_forms")
...
def cancel_all_operations(user_id, chat_id, notify_user=True):
    for prefix in (
        ...
        "pending_deactivation", "pending_batch", "admin_setup",
    ):
...
def start_batch_action(action, user_id, chat_id):
    ...
    if not batches:
        ...
        return
    set_state(
        f"batch_action:{user_id}",
        json.dumps({"action": action, "batch_ids": [batch["id"] for batch in batches]}, ensure_ascii=False),
    )
    instructions = {
        "receive": (
...
def process_batch_selection(text, user_id, chat_id, action_state):
    ...
    stage = {"receive": "receive", "undo": "undo"}[action]
    selected_short_id = batch_id_from_button_text(text)
    allowed_ids = {int(value) for value in action_state.get("batch_ids", [])}
    batch = next(
        (
            item for item in recent_batches(stage)
            if item["short_id"] == selected_short_id
...
def process_private_message(text, user_id, chat_id):
    REQUEST_CONTEXT.chat_id = str(chat_id)
    ...
    try:
        ...
        admin_setup = json.loads(get_state(f"admin_setup:{user_id}", "{}") or "{}")
        if admin_setup.get("step") in {"user_id", "name"}:
            process_admin_setup(text, user_id, chat_id, admin_setup)
            return
        ...
        if text in {"⚙️ مدیریت ربات", "⚙️ مدیریت و تنظیمات"}:
            send_message("⚙️ بخش مدیریت و تنظیمات:", chat_id, management_menu(user_id))
        elif text == "📊 گزارش‌ها":
            send_message("📊 نوع گزارش را انتخاب کنید:", chat_id, reports_menu(user_id))
        elif text == "🎓 مدیریت مدارک":
            send_message("🎓 بخش مدیریت مدارک:", chat_id, certificates_menu(user_id))
        elif text in {"📋 فرم‌ها و مدرس‌ها", "📄 گزارش تک‌فرمی", "🖥 مدارک صادره توسط خودم"}:
            send_message("👨‍🏫 ابتدا مدرس و سپس فرم را انتخاب کنید:", chat_id, forms_teachers_menu())
        elif text == "📚 آموزش کامل کار با ربات":
            send_message("📚 موضوع موردنظر را انتخاب کنید:", chat_id, HELP_MENU)
        elif text in HELP_TEXTS:
            send_message(HELP_TEXTS[text], chat_id, HELP_MENU)
        elif text in {"🏠 منوی اصلی", "🔙 بازگشت به منوی اصلی"}:
            send_message("🏠 منوی اصلی:", chat_id, main_menu(user_id))
            ...
            require_permission(user_id, "multi_reports")
            ...
            if not (has_permission(user_id, "nursing_receive") or has_permission(user_id, "nursing_undo")):
                raise ValueError("⛔ شما به بخش مدارک نظام پرستاری دسترسی ندارید.")
            ...
            require_permission(user_id, "nursing_receive")
            start_batch_action("receive", user_id, chat_id)
        elif text == "↩️ اصلاح انتخاب اشتباه":
            require_permission(user_id, "nursing_undo")
            ...
            require_permission(user_id, "teacher_colors")
            ...
            send_message("⚙️ بخش مدیریت ربات:", chat_id, management_menu(user_id))
        elif text == "👥 مدیریت مدیرها":
            require_permission(user_id, "manage_admins")
            send_message("👥 افزودن یا برکناری مدیرها:", chat_id, ADMIN_MENU)
        elif text == "➕ افزودن مدیر":
            require_permission(user_id, "manage_admins")
            set_state(f"admin_setup:{user_id}", json.dumps({"step": "user_id"}))
            send_message("🆔 آیدی عددی تلگرام مدیر جدید را وارد کنید:", chat_id)
        elif text == "✏️ ویرایش دسترسی مدیر":
            require_permission(user_id, "manage_admins")
            if not list_admins():
                send_message("ℹ️ هنوز مدیر فرعی فعالی ثبت نشده است.", chat_id, ADMIN_MENU)
            else:
                send_message("مدیری را که می‌خواهید ویرایش کنید انتخاب کنید:", chat_id,
                             admins_edit_menu(user_id))
        elif text == "👤 فهرست و برکناری مدیرها":
            require_permission(user_id, "manage_admins")
            admins = list_admins()
            if not admins:
                send_message("ℹ️ هنوز مدیر فرعی فعالی ثبت نشده است.", chat_id, ADMIN_MENU)
            else:
                send_message("مدیر موردنظر برای برکناری را انتخاب کنید:", chat_id, admins_remove_menu(user_id))
        elif text == "♻️ بازیابی موارد غیرفعال":
            if not (has_permission(user_id, "edit_forms") or has_permission(user_id, "manage_teachers")):
                raise ValueError("⛔ اجازه بازیابی موارد غیرفعال را ندارید.")
            ...
            require_permission(user_id, "transfer_forms")
            if form_records:
            ...
            return
        elif text in {"🔍 جست‌وجوی ثبت‌نام‌کنندگان", "🔍 جست‌وجوی افراد"}:
            require_permission(user_id, "search_people")
            ...
            require_permission(user_id, "health")
            ...
            send_message(
                ...
                chat_id,
                teacher_menu(selected_teacher, user_id),
            )
        elif text == "🔙 بازگشت به مدرس":
            teacher_name = get_state(f"selected_teacher:{user_id}", "")
            if teacher_name in teachers:
                send_message(f"👨‍🏫 منوی مدرس «{teacher_name}»:", chat_id, teacher_menu(teacher_name, user_id))
            else:
                send_message("🏠 منوی اصلی:", chat_id, main_menu())
        elif text == "➕ ثبت فرم جدید":
            require_permission(user_id, "add_forms")
            ...
            require_permission(user_id, "view_status")
            send_status()
            ...
            send_message(f"📋 خروجی موردنظر برای فرم «{report_name}» را انتخاب کنید:",
                         chat_id, custom_form_menu(survey_code, user_id))
        elif text == "🎓 ساخت PowerPoint مدارک جدید":
            require_permission(user_id, "self_certificates")
            request_custom_export_confirmation("new", user_id, chat_id)
        elif text == "🆕 فقط ثبت‌های جدید":
            require_permission(user_id, "single_reports")
            request_custom_export_confirmation("new", user_id, chat_id)
        elif text == "📚 همه ثبت‌ها":
            require_permission(user_id, "full_reports")
            ...
            require_permission(user_id, "full_reports" if BUTTON_ACTIONS[text] == "all_full" else "single_reports")
...
def process_multi_callback(data, user_id, chat_id):
    if not (has_permission(user_id, "multi_reports") or has_permission(user_id, "self_certificates")):
        raise ValueError("⛔ شما اجازه خروجی چندفرمی را ندارید.")
...
def process_batch_callback(data, user_id, chat_id):
    ...
    require_permission(user_id, "nursing_undo" if action.startswith("undo") else "nursing_receive")
    ...
    set_state(state_key, "{}")
    if action == "receive":
        items = enrich_batch_contacts(batch["id"], get_batch_items(batch["id"]))
        send_postal_contacts(items, chat_id, f"گزارش {batch['short_id']}")
    messages = {
        "receive": (
...
def process_admin_callback(data, user_id, chat_id):
    require_permission(user_id, "manage_admins")
    if data in {"admin:protected", "admin:protected_edit"}:
        send_message("🔒 این مدیر مجوز مدیریت مدیرها را دارد و فقط مالک اصلی می‌تواند او را برکنار کند.", chat_id)
        return
    if data.startswith("admin:edit:"):
        ...
        if not target:
            raise ValueError("⚠️ مدیر فعال موردنظر پیدا نشد.")
        if not can_modify_admin(user_id, target):
            raise ValueError("⛔ دسترسی مدیر هم‌سطح فقط توسط مالک اصلی قابل ویرایش است.")
        setup = {"step": "permissions", "mode": "edit", "target_id": target_id,
                 "display_name": target["display_name"], "selected": target["permissions"]}
        set_state(f"admin_setup:{user_id}", json.dumps(setup, ensure_ascii=False))
        send_message(f"🔐 دسترسی‌های «{target['display_name']}» را ویرایش کنید:", chat_id,
                     admin_permissions_menu(target["permissions"]))
        return
    if data.startswith("admin:remove:"):
        target_id = data.rsplit(":", 1)[1]
        deactivate_admin(user_id, target_id)
        send_message("✅ مدیر موردنظر برکنار شد.", chat_id, ADMIN_MENU)
        return
    state_key = f"admin_setup:{user_id}"
    setup = json.loads(get_state(state_key, "{}") or "{}")
    if setup.get("step") != "permissions":
        raise ValueError("⌛ روند افزودن مدیر منقضی شده؛ دوباره شروع کنید.")
    selected = set(setup.get("selected", []))
    if data.startswith("adminperm:toggle:"):
        key = data.rsplit(":", 1)[1]
        if key not in PERMISSIONS:
            raise ValueError("⚠️ دسترسی انتخاب‌شده معتبر نیست.")
        selected.symmetric_difference_update({key})
    elif data == "adminperm:all":
        selected = set(PERMISSIONS)
    elif data == "adminperm:clear":
        selected = set()
    elif data == "adminperm:save":
        existing = get_admin(setup["target_id"])
        if existing and not can_modify_admin(user_id, existing):
            raise ValueError("⛔ دسترسی مدیر هم‌سطح فقط توسط مالک اصلی قابل ویرایش است.")
        save_admin(setup["target_id"], setup["display_name"], selected, user_id)
        set_state(state_key, "{}")
        send_message(
            f"✅ مدیر «{setup['display_name']}» ثبت شد. او می‌تواند با آیدی خودش /start را برای ربات بفرستد.",
            chat_id, ADMIN_MENU,
        )
        return
    setup["selected"] = sorted(selected)
    set_state(state_key, json.dumps(setup, ensure_ascii=False))
    send_message(f"🔐 {len(selected)} دسترسی انتخاب شده است.", chat_id, admin_permissions_menu(selected))
...
def process_callback(callback_id, data, user_id, chat_id):
    ...
    try:
        ...
        if data == "cancel":
            ...
            set_state(f"admin_setup:{user_id}", "{}")
        ...
        if data.startswith("batch:"):
            ...
            return
        if data.startswith("admin:") or data.startswith("adminperm:"):
            process_admin_callback(data, user_id, chat_id)
            return
        ...
        if action in {"custom_new", "custom_full"}:
            if pending.get("certificate_type") == "self":
                ...
                require_permission(user_id, "full_reports" if action == "custom_full" else "single_reports")
            execute_custom_export(pending)
        else:
            require_permission(user_id, "full_reports" if action == "all_full" else "single_reports")
...
def telegram_webhook():
    ...
    update_admin_chat(user_id, chat_id)
