...
def run_report(include_processed=False, all_forms=False):
...
def run_single_report(command):
    ...
    try:
        require_settings()
        init_db()
        code, report_name = SINGLE_REPORT_COMMANDS[command]
...
def help_text():
    return (
        ...
        "/report_all_new — پاسخ‌های جدید هر چهار فرم\n"
        "/report_all — همه پاسخ‌های هر چهار فرم (نیازمند تأیید)\n"
        "/status — تعداد کل و جدید هر چهار فرم\n"
    ...
    while True:
        try:
            ...
            for update in telegram_get_updates(offset):
                ...
                valid_commands = {*UTILITY_COMMANDS, *SINGLE_REPORT_COMMANDS}
                if chat_id != str(TELEGRAM_CHAT_ID) or command not in valid_commands:
                    continue
                user_id = str((message.get("from") or {}).get("id", "unknown"))
                process_command_safely(command, user_id)
        except Exception:
            log.exception("Telegram polling failed")
            time.sleep(10)


def handle_report_command(command, user_id="unknown"):
    if command == "/help":
        send_message(help_text())
        return
    if command == "/status":
        send_message("در حال بررسی وضعیت چهار فرم…")
        rows = run_status()
        if rows is None:
    ...
    if command in SINGLE_REPORT_COMMANDS:
        report_name = SINGLE_REPORT_COMMANDS[command][1]
    ...
    if command == "/report_all":
        ...
        send_message(
            "این دستور تمام پاسخ‌های هر چهار فرم را دوباره ارسال می‌کند. "
        ...
        send_message("تأیید شد؛ در حال آماده‌سازی گزارش کامل هر چهار فرم…")
        ...
        send_message("در حال بررسی پاسخ‌های جدید هر چهار فرم…")
...
def process_command_safely(command, user_id="unknown"):
    try:
        handle_report_command(command, user_id)
...
def telegram_webhook():
    ...
    command = text.split()[0].split("@")[0].lower() if text else ""
    valid_commands = {*UTILITY_COMMANDS, *SINGLE_REPORT_COMMANDS}
    if chat_id == str(TELEGRAM_CHAT_ID) and command in valid_commands:
        user_id = str((message.get("from") or {}).get("id", "unknown"))
        threading.Thread(target=process_command_safely, args=(command, user_id), daemon=True).start()
