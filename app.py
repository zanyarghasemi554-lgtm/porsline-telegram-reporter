...
NURSING_MENU = {
    "keyboard": [
        [{"text": "✅ تحویل از نظام پرستاری"}],
        [{"text": "📮 تحویل به پست"}],
        [{"text": "📥 دریافت مجدد گزارش"}],
        [{"text": "↩️ اصلاح وضعیت مدرک"}],
        [{"text": "🔙 بازگشت به منوی اصلی"}],
        [{"text": "❌ انصراف"}],
    ],
    "resize_keyboard": True,
...
def batch_button_label(batch):
    ...
    return f"{batch['short_id']} • {batch['row_count']} نفر • {names[:36]}"
...
def start_batch_action(action, user_id, chat_id):
    ...
    send_message("یکی از گزارش‌ها را انتخاب کنید:", chat_id, menu)
...
def process_batch_selection(text, user_id, chat_id, action_state):
    ...
    send_message(
        f"⚠️ وضعیت تمام {batch['row_count']} نفر گزارش {batch['short_id']} به «{title}» تغییر کند؟",
        chat_id, markup,
    )
...
def process_private_message(text, user_id, chat_id):
    try:
        ...
        if text == "⚙️ مدیریت ربات":
            ...
            send_message("🎓 مدیریت وضعیت مدارک:", chat_id, NURSING_MENU)
        elif text == "✅ تحویل از نظام پرستاری":
            start_batch_action("receive", user_id, chat_id)
        elif text == "📮 تحویل به پست":
            start_batch_action("post", user_id, chat_id)
        elif text == "📥 دریافت مجدد گزارش":
            start_batch_action("redownload", user_id, chat_id)
        elif text == "↩️ اصلاح وضعیت مدرک":
...
def process_batch_callback(data, user_id, chat_id):
    ...
    set_state(state_key, "{}")
    messages = {
        "receive": "✅ وضعیت تمام افراد گزارش به «تحویل از نظام پرستاری» تغییر کرد.",
        "post": "✅ وضعیت تمام افراد گزارش به «تحویل به پست» تغییر کرد.",
        "undo_receive": "✅ وضعیت گزارش به «در انتظار نظام پرستاری» برگشت.",
        "undo_post": "✅ وضعیت گزارش به «تحویل از نظام پرستاری» برگشت.",
    }
