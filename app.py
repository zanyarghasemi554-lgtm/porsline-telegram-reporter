...
def main_menu():
    keyboard = [
        [{"text": "📚 دوره‌های من"}, {"text": "👩‍⚕️ خانم طاهرخانی"}],
        [{"text": "📄 ظهیری"}, {"text": "📊 وضعیت فرم‌ها"}],
        [{"text": "📦 گزارش همه فرم‌ها"}],
        [{"text": "➕ ثبت فرم جدید"}],
    ]
    for _command, survey_code, report_name in get_custom_forms():
...
def process_form_registration(text, user_id, chat_id, registration):
    ...
    if registration.get("step") == "identifier":
        ...
        save_custom_form(custom_form_command(survey_code), survey_code, report_name, user_id)
        set_state(f"form_registration:{user_id}", "{}")
        send_message(
            f"✅ فرم «{report_name}» با موفقیت ثبت شد و دکمه آن به منوی اصلی اضافه شد.",
            chat_id,
            main_menu(),
        )
...
def process_private_message(text, user_id, chat_id):
    try:
        ...
        custom_forms = get_custom_forms()
        selected_custom = next(
            ((survey_code, report_name) for _command, survey_code, report_name in custom_forms
        ...
        if text == "📚 دوره‌های من":
        ...
        elif text == "👩‍⚕️ خانم طاهرخانی":
            send_message("👩‍⚕️ یکی از گزینه‌های خانم طاهرخانی را انتخاب کنید:", chat_id, TAHER_MENU)
            ...
            survey_code, report_name = selected_custom
            set_state(
                f"selected_custom_form:{user_id}",
                json.dumps({"survey_code": survey_code, "report_name": report_name}, ensure_ascii=False),
            )
