...
def format_sent_at(value):
    ...
    return value.astimezone(ZoneInfo(DISPLAY_TIMEZONE)).strftime("%Y-%m-%d، ساعت %H:%M")
        ...
        for _source, survey_code, report_name, teacher_name in records:
            ...
            for raw_row in rows:
                ...
                if registration_matches(person, query):
                    candidates.append((response_key(mapping), person))
            processed = get_processed_times(survey_code, [key for key, _person in candidates])
...
def process_registration_search(text, user_id, chat_id):
    ...
    for index, item in enumerate(matches, start=1):
        ...
        lines.extend([
            ...
            f"📋 فرم: {item['form']}",
            f"👨‍🏫 مدرس: {item['teacher']}",
            f"📤 تاریخ ارسال توسط ربات: {format_sent_at(item['sent_at'])}",
        ])
